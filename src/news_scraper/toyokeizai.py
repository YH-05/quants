...
from .types import TOYOKEIZAI_FEEDS, Article, ScraperConfig
...
def fetch_rss_feed(
    ...
    ValueError
        不明なカテゴリが指定された場合
    """
    if category not in TOYOKEIZAI_FEEDS:
        raise ValueError(
            f"Unknown category: {category}. Valid: {list(TOYOKEIZAI_FEEDS.keys())}"
        )

    url = TOYOKEIZAI_FEEDS[category]

    logger.debug("Fetching Toyokeizai RSS", category=category)
    ...
    logger.info(
        "Toyokeizai RSS fetched", category=category, article_count=len(articles)
    )
    return articles
...
async def async_fetch_rss_feed(
    ...
    logger.debug("Fetching Toyokeizai RSS (async)", category=category)
...
def fetch_multiple_categories(
    ...
    if categories is None:
        categories = list(TOYOKEIZAI_FEEDS.keys())
    ...
    for category in categories:
        try:
            ...
            logger.error(
                "Toyokeizai category fetch failed",
                category=category,
                error=str(e),
            )
