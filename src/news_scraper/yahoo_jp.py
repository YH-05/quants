"""Yahoo!ニュース経済 RSS スクレイパー.

curl-cffi（ブラウザ偽装）+ feedparser（RSS 解析）を使用。
3カテゴリ（business/economy/it）対応。
...
from .types import YAHOO_JP_FEEDS, Article, ScraperConfig
...
def fetch_rss_feed(
    ...
    ValueError
        不明なカテゴリが指定された場合
    """
    if category not in YAHOO_JP_FEEDS:
        raise ValueError(
            f"Unknown category: {category}. Valid: {list(YAHOO_JP_FEEDS.keys())}"
        )

    url = YAHOO_JP_FEEDS[category]

    logger.debug("Fetching Yahoo JP RSS", category=category)
    ...
    logger.info("Yahoo JP RSS fetched", category=category, article_count=len(articles))
    return articles
...
async def async_fetch_rss_feed(
    ...
    logger.debug("Fetching Yahoo JP RSS (async)", category=category)
...
def fetch_multiple_categories(
    ...
    if categories is None:
        categories = list(YAHOO_JP_FEEDS.keys())
    ...
    for category in categories:
        try:
            ...
            logger.error(
                "Yahoo JP category fetch failed",
                category=category,
                error=str(e),
            )
