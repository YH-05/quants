"""Investing.com 日本版 RSS スクレイパー.

curl-cffi（ブラウザ偽装）+ feedparser（RSS 解析）を使用。
5カテゴリ（forex/commodities/stocks/economy/bonds）対応。
UTF-8 日本語フィードをネイティブに処理する。
"""
...
from .types import INVESTING_JP_FEEDS, Article, ScraperConfig
...
def fetch_rss_feed(
    ...
    ValueError
        不明なカテゴリが指定された場合
    """
    if category not in INVESTING_JP_FEEDS:
        raise ValueError(
            f"Unknown category: {category}. Valid: {list(INVESTING_JP_FEEDS.keys())}"
        )

    url = INVESTING_JP_FEEDS[category]

    logger.debug("Fetching Investing.com JP RSS", category=category)
...
async def async_fetch_rss_feed(
    ...
    logger.debug("Fetching Investing.com JP RSS (async)", category=category)
    ...
    logger.info(
        "Investing.com JP RSS fetched (async)",
        category=category,
        article_count=len(articles),
    )
    return articles
...
def fetch_multiple_categories(
    ...
    if categories is None:
        categories = list(INVESTING_JP_FEEDS.keys())
    ...
    for category in categories:
        try:
            ...
            logger.error(
                "Investing.com JP category fetch failed",
                category=category,
                error=str(e),
            )
