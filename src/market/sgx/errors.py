...
various error conditions specific to SGX (Singapore Exchange) operations,
including API response failures, rate limiting, response parsing errors,
and data validation errors.

Exception Hierarchy
-------------------
SgxError (base, inherits Exception)
...
class SgxError(Exception):
    ...
    All SGX-specific exceptions inherit from this class,
    providing a single catch point for callers that need to handle
    any SGX-related failure generically.
    ...
    --------
    >>> try:
    ...     raise SgxError("SGX operation failed")
    ... except SgxError as e:
    ...     print(e.message)
    SGX operation failed
    """
...
class SgxRateLimitError(SgxError):
...
class SgxValidationError(SgxError):
