...
class IdxError(Exception):
    ...
    All IDX-specific exceptions inherit from this class,
    providing a single catch point for callers that need to handle
    any IDX-related failure generically.
    ...
    --------
    >>> try:
    ...     raise IdxError("IDX operation failed")
...
class IdxRateLimitError(IdxError):
...
class IdxValidationError(IdxError):
