...
対象モジュール: src/news_scraper/jpx.py
"""
...
from news_scraper.types import JPX_FEEDS, Article, ScraperConfig
...
class TestFetchRssFeed:
    ...
    def test_正常系_複数エントリで記事リストを返す(self) -> None:
        ...
        entry1.get = lambda k, d="": {
            "title": "JPXニュースリリース1",
            "link": "https://www.jpx.co.jp/news/1",
        ...
        entry2.get = lambda k, d="": {
            "title": "JPXニュースリリース2",
            "link": "https://www.jpx.co.jp/news/2",
        ...
        assert result[0].category == "news_release"
    ...
    def test_正常系_カテゴリが記事に設定される(self) -> None:
        ...
        entry.get = lambda k, d="": {
            "title": "テスト記事",
            "link": "https://www.jpx.co.jp/news/3",
            "published": "",
            "summary": "",
        }.get(k, d)
        ...
        with patch("news_scraper.jpx.feedparser.parse", return_value=mock_feed):
            result = fetch_rss_feed(mock_session, "listing")

        assert result[0].category == "listing"
        assert result[0].source == "jpx"

    def test_正常系_URLが正しく構築される(self) -> None:
        """JPX_FEEDS の URL が HTTP リクエストに使用されることを確認。"""
        from news_scraper.jpx import fetch_rss_feed
        ...
        with patch("news_scraper.jpx.feedparser.parse", return_value=mock_feed):
            fetch_rss_feed(mock_session, "news_release")

        called_url = mock_session.get.call_args[0][0]
        assert called_url == JPX_FEEDS["news_release"]
...
class TestAsyncFetchRssFeed:
    ...
    async def test_異常系_不明カテゴリでValueError(self) -> None:
        """不明なカテゴリを指定したとき ValueError が発生することを確認。"""
        from news_scraper.jpx import async_fetch_rss_feed
    ...
    async def test_正常系_複数エントリで記事リストを返す(self) -> None:
        ...
        entry1.get = lambda k, d="": {
            "title": "非同期JPX記事1",
            "link": "https://www.jpx.co.jp/news/10",
            "published": "Mon, 01 Jan 2026 00:00:00 +0900",
            "summary": "非同期要約1",
        }.get(k, d)
        ...
        with patch("news_scraper.jpx.feedparser.parse", return_value=mock_feed):
            result = await async_fetch_rss_feed(mock_session, "news_release")
...
class TestFetchMultipleCategories:
    ...
    def test_正常系_全カテゴリ取得(self) -> None:
        ...
        from news_scraper.jpx import fetch_multiple_categories

        mock_session = MagicMock()

        with (
            patch("news_scraper.jpx.fetch_rss_feed") as mock_fetch,
            patch("news_scraper.jpx.time.sleep"),
        ):
            mock_fetch.return_value = [
                Article(
                    title="記事",
                    url="https://www.jpx.co.jp/1",
                    source="jpx",
        ...
        assert not df.empty
        assert mock_fetch.call_count == len(JPX_FEEDS)
    ...
    def test_正常系_エラーカテゴリをスキップして残りを取得(self) -> None:
        ...
        with (
            ...
            df = fetch_multiple_categories(mock_session, categories=["news_release"])
...
class TestAsyncFetchMultipleCategories:
    ...
    async def test_正常系_空カテゴリリストで空DataFrameを返す(self) -> None:
        """空のカテゴリリストを渡したとき空 DataFrame を返すことを確認。"""
        from news_scraper.jpx import async_fetch_multiple_categories
    ...
    async def test_正常系_指定カテゴリで記事を取得(self) -> None:
        ...
        with patch("news_scraper.jpx.async_fetch_rss_feed") as mock_fetch:
            mock_fetch.return_value = [
                Article(
                    title="非同期記事",
                    url="https://www.jpx.co.jp/1",
