"""Tests for market.hose.constants module.
...
from market.hose.constants import (
    CURRENCY,
    DEFAULT_OUTPUT_DIR,
    EXCHANGE_CODE,
    EXCHANGE_NAME,
...
class TestModuleExports:
    def test_正常系_モジュールがインポートできる(self) -> None:
        from market.hose import constants
...
class TestExchangeIdentification:
    def test_正常系_EXCHANGE_CODEがHOSE(self) -> None:
        assert EXCHANGE_CODE == "HOSE"

    def test_正常系_EXCHANGE_NAMEがHo_Chi_Minh_Stock_Exchange(self) -> None:
        assert EXCHANGE_NAME == "Ho Chi Minh Stock Exchange"

    def test_正常系_SUFFIXがドットVN(self) -> None:
        assert SUFFIX == ".VN"

    def test_正常系_SUFFIXがドットで始まる(self) -> None:
        assert SUFFIX.startswith(".")

    def test_正常系_CURRENCYがVND(self) -> None:
        assert CURRENCY == "VND"
...
class TestOutputConstants:
    def test_正常系_DEFAULT_OUTPUT_DIRが正しい値(self) -> None:
        assert DEFAULT_OUTPUT_DIR == "data/raw/hose/"
