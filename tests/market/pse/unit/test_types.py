...
class TestPseConfig:
    ...
    def test_正常系_デフォルト値で初期化できる(self) -> None:
        config = PseConfig()
        assert config.exchange_code == "PSE"
        assert config.suffix == ".PS"
        assert config.timeout == 30.0

    def test_正常系_カスタム値で初期化できる(self) -> None:
        config = PseConfig(timeout=60.0)
