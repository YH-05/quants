...
class TestOutputConstants:
    ...
    def test_正常系_DEFAULT_OUTPUT_SUBDIRがrawを含む(self) -> None:
        """DEFAULT_OUTPUT_SUBDIR が raw パスを含むこと。"""
        assert "raw" in DEFAULT_OUTPUT_SUBDIR

    def test_正常系_DEFAULT_OUTPUT_SUBDIRがnasdaqを含む(self) -> None:
        """DEFAULT_OUTPUT_SUBDIR が nasdaq を含むこと。"""
        assert "nasdaq" in DEFAULT_OUTPUT_SUBDIR

    def test_正常系_DEFAULT_OUTPUT_SUBDIRが正しい値(self) -> None:
        """DEFAULT_OUTPUT_SUBDIR が設計通りの値であること（data/ プレフィックスなし）。"""
        assert DEFAULT_OUTPUT_SUBDIR == "raw/nasdaq"
