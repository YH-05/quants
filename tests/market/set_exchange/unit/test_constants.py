"""Tests for market.set_exchange.constants module.
...
class TestModuleExports:
    def test_正常系_モジュールがインポートできる(self) -> None:
        from market.set_exchange import constants
...
class TestExchangeIdentification:
    def test_正常系_EXCHANGE_CODEがSET(self) -> None:
        assert EXCHANGE_CODE == "SET"

    def test_正常系_EXCHANGE_NAMEがStock_Exchange_of_Thailand(self) -> None:
        assert EXCHANGE_NAME == "Stock Exchange of Thailand"

    def test_正常系_SUFFIXがドットBK(self) -> None:
        assert SUFFIX == ".BK"

    def test_正常系_SUFFIXがドットで始まる(self) -> None:
        assert SUFFIX.startswith(".")

    def test_正常系_CURRENCYがTHB(self) -> None:
        assert CURRENCY == "THB"
