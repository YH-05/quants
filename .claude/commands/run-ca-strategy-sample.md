---
description: DIS 1銘柄のサンプル実行を行うオーケストレーターコマンド。agent_io.py の Python 関数と transcript-claim-extractor/transcript-claim-scorer エージェントを組み合わせた 6 ステップのパイプラインを実行します。
argument-hint: [TICKER]
---

# /run-ca-strategy-sample - CA Strategy サンプル実行

DIS 1 銘柄を対象とした CA Strategy end-to-end サンプルパイプラインを実行するコマンドです。`ca-strategy-lead`（Agent Teams 方式）は使わず、コマンドがオーケストレーターとして全体フローを制御します。
...
|------|-----|
| TICKER | DIS（引数で上書き可） |
| workspace | `research/ca_strategy_poc/workspace_dis_sample` |
...
| dogma_path | `analyst/Competitive_Advantage/analyst_YK/dogma.md` |
...
Step 4: Phase 2 入力準備 + エージェント呼び出し（transcript-claim-scorer）
...
3. **workspace ディレクトリの作成**
   ...
   以下の Bash コマンドを実行する:
   ```bash
   mkdir -p research/ca_strategy_poc/workspace_dis_sample/phase1_output/{TICKER}
   mkdir -p research/ca_strategy_poc/workspace_dis_sample/phase2_output/{TICKER}
   mkdir -p research/ca_strategy_poc/workspace_dis_sample/output
   mkdir -p research/ca_strategy_poc/workspace_dis_sample/checkpoints
   ```
...
- `research/ca_strategy_poc/workspace_dis_sample/extraction_input.json` が生成されること
...
指示:
  以下のパスにある extraction_input.json を読み込み、DIS の決算トランスクリプトから
  競争優位性の主張を抽出して extraction_output.json を書き出してください。

  extraction_input.json パス:
    research/ca_strategy_poc/workspace_dis_sample/extraction_input.json

  workspace_dir:
    research/ca_strategy_poc/workspace_dis_sample

  出力先:
    research/ca_strategy_poc/workspace_dis_sample/phase1_output/{TICKER}/extraction_output.json
...
**成功判定**:
- `research/ca_strategy_poc/workspace_dis_sample/phase1_output/{TICKER}/extraction_output.json` が生成されること
...
output_path = Path('research/ca_strategy_poc/workspace_dis_sample/phase1_output/{TICKER}/extraction_output.json')
...
**成功判定**:
- コマンドが終了コード 0 で完了すること
- `research/ca_strategy_poc/workspace_dis_sample/scoring_input.json` が生成されること

#### Step 4.2: Phase 2 エージェント呼び出し

`Task(transcript-claim-scorer)` を使用して確信度スコアリングを実行する。

```
Task: transcript-claim-scorer
指示:
  以下のパスにある scoring_input.json を読み込み、Phase 1 で抽出された DIS の主張に
  確信度スコアを付与して scoring_output.json を書き出してください。

  scoring_input.json パス:
    research/ca_strategy_poc/workspace_dis_sample/scoring_input.json

  workspace_dir:
    research/ca_strategy_poc/workspace_dis_sample

  出力先:
    research/ca_strategy_poc/workspace_dis_sample/scoring_output.json

  注意:
  - scoring_input.json を最初に Read して ticker・phase1_output_dir・kb1_dir・kb2_dir・kb3_dir・workspace_dir を取得すること
  ...
  - PoiT 制約（cutoff_date=2015-09-30）を厳守すること
  - 4段階評価（ゲートキーパー→KB1-T→KB2-T→KB3-T）を全主張に適用すること
  - 各主張に final_confidence（0.1-0.9 の範囲）を付与すること
  - 出力先は {workspace_dir}/scoring_output.json であること
```

**成功判定**:
- `research/ca_strategy_poc/workspace_dis_sample/scoring_output.json` が生成されること

**失敗時**: エージェントのエラー出力を確認し、Phase 1 出力ファイルが正しく参照できているか確認する。
...
phase1_path = Path('research/ca_strategy_poc/workspace_dis_sample/phase1_output/{TICKER}/extraction_output.json')
claims = validate_extraction_output(
...
scoring_path = Path('research/ca_strategy_poc/workspace_dis_sample/scoring_output.json')
...
### Step 2/4.2: エージェント呼び出し失敗

```
エラー: エージェント {エージェント名} が失敗しました

確認事項:
- extraction_input.json / scoring_input.json が正しく生成されているか
- KB ファイルへのパスが正しいか（kb_base_dir: analyst/transcript_eval）
- dogma.md のパスが正しいか（analyst/Competitive_Advantage/analyst_YK/dogma.md）
```
...
### 生成ファイル
- extraction_input.json: research/ca_strategy_poc/workspace_dis_sample/extraction_input.json
- extraction_output.json: research/ca_strategy_poc/workspace_dis_sample/phase1_output/{TICKER}/extraction_output.json
- scoring_input.json: research/ca_strategy_poc/workspace_dis_sample/scoring_input.json
- scoring_output.json: research/ca_strategy_poc/workspace_dis_sample/scoring_output.json
```
...
| `transcript-claim-scorer` | Step 4.2 | KB1-T/KB2-T/KB3-T+dogma.mdで4段階評価を実行し scoring_output.json を生成 |
...
- **dogma.md**: `analyst/Competitive_Advantage/analyst_YK/dogma.md`
