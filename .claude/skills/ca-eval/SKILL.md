...
| Dogma | `analyst/Competitive_Advantage/analyst_YK/dogma/dogma_v1.0.md` | 1 | 全フェーズ |
...
```
analyst/research/CA_eval_{YYYYMMDD}-{HHMM}_{TICKER}/
├── 00_meta/
│   └── research-meta.json
├── 01_data_collection/
│   ├── sec-data.json              <- T1
...
評価完了後、結果をナレッジグラフに投入できます:

```bash
/emit-graph-queue --command ca-eval --input analyst/research/CA_eval_{id}/02_claims/claims.json
/save-to-graph --source ca-eval
```
