...
```python
from finance.utils.profiling import profile
...
```python
from finance.utils.profiling import timeit
...
```python
from finance.utils.profiling import compare_performance
...
class FundIdResolver:
    ...
    Tier 1: In-memory cache（最速、セッション中に構築）
    Tier 2: File cache（JSON ファイル、プロセス再起動後も有効）
    Tier 3: API call（キャッシュミス時のフォールバック）
    """
