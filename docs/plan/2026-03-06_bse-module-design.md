...
**スコープ:** BSE API（20エンドポイント）のみ。NSE/MCA/Listing Centre は将来の拡張として別途対応。

**取得対象データ:**
- ヒストリカル株価（OHLCV）
- Bhavcopy（日次全銘柄集約データ）
- 企業情報・財務データ（決算スナップショット、コーポレートアクション）
- 企業開示情報（アナウンスメント）
- インデックスデータ（SENSEX 等）
...
| 株価 | `/StockTrading/w` | 取引統計 |
| Bhavcopy | `/download/BhavCopy/Equity/...CSV` | エクイティ日次集約 |
| Bhavcopy | `/download/Bhavcopy/Derivative/...CSV` | デリバティブ日次集約 |
...
| インデックス | `/FillddlIndex/w` | 指数名一覧 |
| 銘柄検索 | `/PeerSmartSearch/w` | シンボル検索（HTML） |
| 銘柄検索 | `/ListofScripData/w` | 銘柄一覧 |
| 銘柄検索 | `/BindDDLEQ/w` | 銘柄グループ一覧 |
...
- `fetch_52w_high_low(scrip_code) -> dict[str, float]`
...
- `fetch_daily(index_name, date) -> pd.DataFrame`
...
| `bse/collectors/__init__.py` | コレクター再エクスポート |
