...
- [ ] 全4コンポーネントの移植が完了し、`make check-all` が成功すること
- [ ] 既存テスト（database, market, utils_core）が全て PASS すること
- [ ] 既存の DuckDBClient / BloombergFetcher の公開 API に破壊的変更がないこと
...
- [ ] DuckDB upsert ロジックの移植（store_df / get_table_names）
  - Issue: [#3631](https://github.com/YH-05/finance/issues/3631)
  - ステータス: todo
  - 見積もり: 1.5h

- [ ] ProjectConfig frozen dataclass の作成
...
- [ ] Bloomberg 型定義・定数・エクスポートの追加
  - Issue: [#3633](https://github.com/YH-05/finance/issues/3633)
  - ステータス: todo
  - 依存: Wave 1
  - 見積もり: 0.5h

- [ ] Bloomberg _process_* プレースホルダ実装とテスト
  - Issue: [#3634](https://github.com/YH-05/finance/issues/3634)
  - ステータス: todo
  - 依存: task-3
  - 見積もり: 2h

- [ ] Bloomberg 新規メソッド追加・store_to_database DuckDB対応・norecursedirs 削除
...
- [ ] News Embedding 型定義・リーダー・レートリミッターの作成
  - Issue: [#3636](https://github.com/YH-05/finance/issues/3636)
  - ステータス: todo
  - 見積もり: 1h

- [ ] News Embedding エクストラクター・ChromaDB ストア・パイプラインの作成
  - Issue: [#3637](https://github.com/YH-05/finance/issues/3637)
  - ステータス: todo
  - 依存: task-6
  - 見積もり: 1.5h

- [ ] News Embedding CLI・エントリポイント・pyproject.toml 統合
